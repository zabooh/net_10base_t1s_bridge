...
# BridgeGUITelnet._restore_semantic_colors for why these are needed at all.
RED_DARK = "#ff6b6b"
GREEN_DARK = "#4ac94a"
MUTED_DARK = "#9a9a9a"
RED_LIGHT = "#b00000"
...
class BridgeGUITelnet:
    def __init__(self, root: tk.Tk, dark: bool = True):
        ...
        self.root = root
        self.root.title("Bridge Status & Configuration (Telnet)")
        ...
        # where 'zoomed' were ever unavailable. Windows-only tool (see the ctypes/DWM
        # title bar code below), so no cross-platform fallback for the state name.
    ...
    def setup_ui(self):
        ...
        # COM port selection - everything below this (bulk buttons, connection
        # indicator, status label, notebook tabs) is unchanged.
        top_frame = ttk.Frame(self.root)
        top_frame.pack(fill=tk.X, padx=5, pady=5)

        ttk.Label(top_frame, text="IP:").pack(side=tk.LEFT)
        self.ip_var = tk.StringVar(value=self.config.get("ip", "192.168.0.12"))
        ttk.Entry(top_frame, textvariable=self.ip_var, width=15).pack(side=tk.LEFT, padx=5)

        ttk.Label(top_frame, text="User:").pack(side=tk.LEFT)
        self.telnet_user_var = tk.StringVar(value=self.config.get("telnet_user", "admin"))
        ttk.Entry(top_frame, textvariable=self.telnet_user_var, width=10).pack(side=tk.LEFT, padx=5)
    ...
    def create_terminal_tab(self):
        ...
        self.terminal_link: Optional[TelnetLink] = None
...
WORKFLOW:
1. Enter the device's IP address, Telnet user and password, then Connect
...
Example commands (Quick Commands buttons, or typed in the Terminal tab):
...
"""
    ...
    def connect_device(self):
        ...
        host = self.ip_var.get().strip()
        if not host:
            messagebox.showwarning("Warning", "Please enter the device's IP address first")
            return
    ...
    def disconnect_device(self):
        """Close the Telnet connection (for CLI + Terminal)"""
        if self.port_link:
    ...
    def flash_current_hex(self):
        ...
        Independent of the open Telnet link: pyOCD talks to the EDBG probe's SWD
        interface over USB, not the network connection, so nothing needs to
        disconnect first.
        """
...
def main():
    ...
    gui = BridgeGUITelnet(root, dark=not args.light)
    root.mainloop()
