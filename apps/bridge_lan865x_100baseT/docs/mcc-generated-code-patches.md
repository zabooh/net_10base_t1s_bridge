...
**Already fixed identically elsewhere** — the sister project
(`t1s_100baset_bridge`, `docs/PORTING_SOFTWARE.md` §3) hit and fixed this
exact bug the same way, and this repo's own `apps\follower_lan865x` was
already at `3u` (correctly ported from the sister's `follower/`, which "has
had this at 3u since its first commit"). Only this bridge app's own copy had
